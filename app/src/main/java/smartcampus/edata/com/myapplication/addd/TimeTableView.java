package smartcampus.edata.com.myapplication.addd;
...
import smartcampus.edata.com.myapplication.R;
import smartcampus.edata.com.myapplication.Time;
...
public class TimeTableView extends LinearLayout {
    ...
    //最大节数
    public final static int MAXNUM = 8;
    //显示到星期几
...
//    private final static int TIME_TABLE_HEIGHT = 50;
    private final static int TIME_TABLE_HEIGHT = 80;
    /**
     * 线的高度
     */
...
//    private List<TimeTableModel> mListTimeTable = new ArrayList<TimeTableModel>();
    ...
    private void layoutContentView(int week) {
        List<Time> weekClassList = findWeekClassList(week);
        //添加
     ...
     */
    @NonNull
    private List<Time> findWeekClassList(int week) {
        List<Time> list = new ArrayList<>();
        for (Time timeTableModel : mListTimeTable) {
            if (Integer.valueOf(timeTableModel.getWeek()) == week) {
...
//        });
     ...
     * @param week     周
     */
    private LinearLayout createWeekTimeTableView(List<Time> weekList, int week) {
        ...
        if (weekList.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                Time tableModel = weekList.get(i);
                if (i == 0) {
                    //添加的0到开始节数的空格
    ...
    private View createClassView(final Time model) {
        ...
        mTimeTableNameView.setText(model.getContent() );
//        mTimeTableNameView.setText(model.getName() + "@" + model.getClassroom());
        ...
        mTimeTableView.setBackgroundDrawable(getContext().getResources()
                .getDrawable(colors[getColorNum(model.getContent())]));
...
//                Toast.makeText(getContext(), model.getName() + "@" + model.getClassroom(), Toast.LENGTH_LONG).show();
                Toast.makeText(getContext(), model.getContent(), Toast.LENGTH_LONG).show();
            }
        });
...
//        invalidate();
//    }
    public void setTimeTable(List<Time> mlist) {
        this.mListTimeTable = mlist;
        for (Time timeTableModel : mlist) {
            addTimeName(timeTableModel.getContent());
        }
        initView();
        invalidate();
    }
